...
class MARS(BaseBlock):
    ...
    def key_setup(self):
        ...
        for j in range(4):
            ...
            for i in range(15):
                t[i] = rotl(t[i] + self.self.s[511 & t[(i + 14) % 15]], 9)
            for i in range(10):
                self.k[10 * j + i] = t[(4 * i) % 15]

        for i in range(5, 36, step=2):
            j = self.k[i] & 3
            w = self.k[i] | 3
            m = self.make_mask(w)
            p = rotl(self.self.s[j + 256], 31 & self.k[i - 1])
            self.k[i] = w ^ (p & m)
    ...
    def f_trans(self, a, b, c, d, i):
        out1, out2, out3 = self._e(a, self.k[2 * i + 4], self.k[2 * i + 5])
        a = rotl(a, 13)
        c += out2
        b += out1
        d = d ^ out3
    ...
    def encrypt(self, text):
        ...
        for i in range(16):
            d = deque(self.f_trans(d[0], d[1], d[2], d[3], i))
            if i < 8:
                d.rotate(1)
            if i == 7 or i == 15:
                d[1], d[3] = d[3], d[1]
        for i in range(8):
            d = deque(self.b_mix(d[0], d[1], d[2], d[3]))
            if i == 1 or i == 5:
                d[1] -= d[0]
            if i == 2 or i == 6:
                d[1] -= d[2]
    ...
    def decrypt(self, text):
        ...
        for i in range(16):
            d = deque(self.f_trans(d[0], d[1], d[2], d[3], 15 - i))
            if i < 8:
                d.rotate(1)
            if i == 7 or i == 15:
                d[1], d[3] = d[3], d[1]
