...
class Anubis(BaseBlock):
    def __init__(self, key_length: int, key, rounds):
        ...
        self.round_key_enc = None
        self.round_key_dec = None
        self.rounds = rounds

    def key_setup(self, key):
        i, pos = 0, 0
        n = self.key_length >> 5
        kappa, inter = [0] * 10, [0] * 10
        round_key_enc = [[0] * 4 for i in range(n + 9)]
        round_key_dec = [[0] * 4 for i in range(n + 9)]
        while i < n:
            kappa[i] = (key[pos] << 24) ^ (key[pos + 1] << 16) ^ (key[pos + 2] << 8) ^ (key[pos + 3])
            i += 1
            pos += 4

        for r in range(n + 9):
            k0 = self.T4[(kappa[n - 1] >> 24)]
            k1 = self.T4[(kappa[n - 1] >> 16) & 0xff]
            k2 = self.T4[(kappa[n - 1] >> 8) & 0xff]
            k3 = self.T4[(kappa[n - 1]) & 0xff]
            for i in range(n - 2, -1, -1):
                k0 = self.T4[kappa[i] >> 24] ^ (self.T5[k0 >> 24] & 0xff000000) ^ \
                     (self.T5[(k0 >> 16) & 0xff] & 0x00ff0000) ^ (self.T5[(k0 >> 8) & 0xff] & 0x0000ff00) ^ \
                     self.T5[k0 & 0xff] & 0x000000ff
                k1 = self.T4[kappa[i] >> 24] ^ (self.T5[k1 >> 24] & 0xff000000) ^ \
                     (self.T5[(k1 >> 16) & 0xff] & 0x00ff0000) ^ (self.T5[(k1 >> 8) & 0xff] & 0x0000ff00) ^ \
                     self.T5[k1 & 0xff] & 0x000000ff
                k2 = self.T4[kappa[i] >> 24] ^ (self.T5[k2 >> 24] & 0xff000000) ^ \
                     (self.T5[(k2 >> 16) & 0xff] & 0x00ff0000) ^ (self.T5[(k2 >> 8) & 0xff] & 0x0000ff00) ^ \
                     self.T5[k2 & 0xff] & 0x000000ff
                k3 = self.T4[kappa[i] >> 24] ^ (self.T5[k3 >> 24] & 0xff000000) ^ \
                     (self.T5[(k3 >> 16) & 0xff] & 0x00ff0000) ^ (self.T5[(k3 >> 8) & 0xff] & 0x0000ff00) ^ \
                     self.T5[k3 & 0xff] & 0x000000ff
            ...
            for i in range(n):
                j = i
                inter[i] = self.T0[(kappa[j - 1] >> 24)]
                if j < 0:
                    j = n - 1
                else:
                    j -= 1
                inter[i] ^= self.T1[(kappa[j - 1] >> 16) & 0xff]
                if j < 0:
                    j = n - 1
                else:
                    j -= 1
                inter[i] ^= self.T2[(kappa[j - 1] >> 8) & 0xff]
                if j < 0:
                    j = n - 1
                else:
                    j -= 1
                inter[i] ^= self.T3[(kappa[j]) & 0xff]
            kappa[0] = inter[0] ^ self.rc[r]

        for i in range(4):
            round_key_dec[0][i] = round_key_enc[self.rounds][i]
            self.round_key_dec[self.rounds][i] = self.round_key_enc[0][i]

        for r in range(self.rounds):
            for i in range(4):
                v = round_key_enc[self.rounds - r][i]
                round_key_dec = self.T0[self.T4[(v >> 24)] & 0xff] ^ self.T1[self.T4[(v >> 16) & 0xff] & 0xff] ^ \
                                self.T2[self.T4[(v >> 8) & 0xff] & 0xff] ^ self.T3[self.T4[v & 0xff] & 0xff]

        self.round_key_enc = round_key_enc
        self.round_key_dec = round_key_dec

    def _crypt(self, text, round_key):
        i, pos = 0, 0
        state, inter = [0] * 4, [0] * 4
        ciphertext = [0] * 4
        while i < 4:
            state[i] = (text[pos] << 24) ^ (text[pos + 1] << 16) ^ \
                       (text[pos + 2] << 8) ^ (text[pos + 3]) ^ \
                       round_key[0][i]
            i += 1
            pos += 4
        for r in range(self.key_length >> 5 + 9):
            inter[0] = self.T0[(state[0] >> 24)] ^ self.T1[(state[1] >> 24)] ^ \
                       self.T2[(state[2] >> 24)] ^ self.T3[(state[3] >> 24)] ^ round_key[r][0]
            inter[1] = self.T0[(state[0] >> 16) & 0xff] ^ self.T1[(state[1] >> 16) & 0xff] ^ \
                       self.T2[(state[2] >> 16) & 0xff] ^ self.T3[(state[3] >> 16) & 0xff] ^ round_key[r][1]
            inter[2] = self.T0[(state[0] >> 8) & 0xff] ^ self.T1[(state[1] >> 8) & 0xff] ^ \
                       self.T2[(state[2] >> 8) & 0xff] ^ self.T3[(state[3] >> 8) & 0xff] ^ round_key[r][2]
            inter[3] = self.T0[(state[0]) & 0xff] ^ self.T1[(state[1]) & 0xff] ^ \
                       self.T2[(state[2]) & 0xff] ^ self.T3[(state[3]) & 0xff] ^ round_key[r][3]
        ...
        inter[0] = (self.T0[(state[0] >> 24)] & 0xff000000) ^ (self.T1[(state[1] >> 24)] & 0x00ff0000) ^ \
                   (self.T2[(state[2] >> 24)] & 0x0000ff00) ^ (self.T3[(state[3] >> 24)] & 0x000000ff) ^ \
                   round_key[self.rounds][0]
        inter[1] = (self.T0[(state[0] >> 16) & 0xff] & 0xff000000) ^ (self.T1[(state[1] >> 16) & 0xff] & 0x00ff0000) ^ \
                   (self.T2[(state[2] >> 16) & 0xff] & 0x0000ff00) ^ (self.T3[(state[3] >> 16) & 0xff] & 0x000000ff) ^ \
                   round_key[self.rounds][1]
        inter[2] = (self.T0[(state[0] >> 8) & 0xff] & 0xff000000) ^ (self.T1[(state[1] >> 8) & 0xff] & 0x00ff0000) ^ \
                   (self.T2[(state[2] >> 8) & 0xff] & 0x0000ff00) ^ (self.T3[(state[3] >> 8) & 0xff] & 0x000000ff) ^ \
                   round_key[self.rounds][2]
        inter[3] = (self.T0[(state[0]) & 0xff] & 0xff000000) ^ (self.T1[(state[1]) & 0xff] & 0x00ff0000) ^ \
                   (self.T2[(state[2]) & 0xff] & 0x0000ff00) ^ (self.T3[(state[3]) & 0xff] & 0x000000ff) ^ \
                   round_key[self.rounds][3]

        i, pos = 0, 0
        while i < 4:
            w = inter[i]
            ciphertext[pos] = w >> 24
            ciphertext[pos + 1] = w >> 16
            ciphertext[pos + 2] = w >> 8
            ciphertext[pos + 3] = w
            i += 1
            pos += 4
    ...
    def _pad(self, text):
        if len(text) % 4 == 0:
            ...
            diff = 4 - (len(text) % 4)
            return text + b'\0' * diff

    def _unpad(self, text):
        if len(text) % 4 == 0:
            return text
        else:
            diff = int(text[len(text) - 1])
            message = text[:len(text) - 1 - diff]
            return message

    def encrypt(self, text):
        text = self._pad(text)
        return self._crypt(text, self.round_key_enc)

    def decrypt(self, text):
        text = self._crypt(text, self.round_key_dec)
        text = self._unpad(text)
        return text
