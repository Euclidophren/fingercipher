...
class RC2:
    ...
    def __init__(self, key):
        ...
        for i in range(128):
            if len(key) > i:
                ...
                rc2_key[i] = self.sbox[rc2_key[i - 1] + rc2_key[i - len(key)] & 0xFF]

        rc2_key[128 - len(key)] = self.sbox[rc2_key[128 - len(key)]]

        if len(key) < 128:
            for i in range(127 - len(key), -1, -1):
                xor = rc2_key[i + 1] ^ rc2_key[len(key) + i]
                rc2_key[i] = self.sbox[xor & 0xFF]
    ...
    def block_encrypt(self, input_buffer):
        ...
        for i in range(16):
            if i % 4:
                R[0], R[1], R[2], R[3] = self.encrypt_mixup(self.K, R[0], R[1], R[2], R[3], i)
            else:
    ...
    def block_decrypt(self, input_buffer):
        R = array.array('H')
        R.frombytes(input_buffer)
        for i in range(16, step=-1):
            if i % 4:
                R[0], R[1], R[2], R[3] = self.decrypt_mixup(self.K, R[0], R[1], R[2], R[3], i)
            else:
