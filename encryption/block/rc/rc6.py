...
class RC6(BaseBlock):
    ...
    def _encrypt_binBlock(self, message, w=32, r=20):
        mod = 2 ** w
        S = self.keysTable
    ...
    def _decrypt_binBlock(self, message, w=32, r=20):
        mod = 2 ** w
        S = self.keysTable
    ...
    def encrypt(self, message, key, w=32, r=20):
        ...
        for block in message:
            output += self._encrypt_binBlock(block, w, r)
        return output

    def decrypt(self, message, key, w=32, r=20):
        message = [message[x * w * 4: (x + 1) * w * 4] for x in range(int(len(message) / (w * 4)))]

        output = ''
        for block in message:
            output += self._decrypt_binBlock(block, w=32, r=20)
