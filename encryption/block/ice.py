...
class IceKey(object):
    ...
    def __init__(self, n: int, key: bytes):
        if self.__rSBOX_INITIALISED != True:
            ...
            for i in range(0, 1024):
                ...
                self.__rSBOX[0][i] = self._perm32(
                    self._gf_exp7(col ^ self.__rSXOR[0][row], self.__rSMOD[0][row]) << RINT(24))
                self.__rSBOX[1][i] = self._perm32(
                    self._gf_exp7(col ^ self.__rSXOR[1][row], self.__rSMOD[1][row]) << RINT(16))
                self.__rSBOX[2][i] = self._perm32(
                    self._gf_exp7(col ^ self.__rSXOR[2][row], self.__rSMOD[2][row]) << RINT(8))
        ...
        if self.__rROUNDS == 8:
            ...
            for i in range(0, self.__rSIZE):
                ...
                for j in range(0, 4):
                    j = RINT(j)
                    kb[RINT(3) - j] = (key[i * RINT(8) + j * RINT(2)] << RINT(8)) | key[
                        i * RINT(8) + j * RINT(2) + RINT(1)]
                for l in range(0, 8):
                    l = RINT(l)
                    kr = self.__rKEYROT[l]
                    for m in range(3):
    ...
    def _ice_f(self, p: RINT, sk: dict) -> int:
        ...
        al ^= sk[0]
        ar ^= sk[1]
        return self.__rSBOX[0][al >> RINT(10)] | self.__rSBOX[1][al & RINT(0x3FF)] | self.__rSBOX[2][ar >> RINT(10)] | \
               self.__rSBOX[3][ar & RINT(0x3FF)]
